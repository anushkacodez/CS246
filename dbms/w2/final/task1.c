...
void solve()
{
    struct ProductDetails productArray[PRODUCTS_NUMBER];
    int num=0;

    FILE *f2=fopen("220101014.csv", "w");

    if(readCSV(productArray, &num))
    {
        fprintf(f2, "%s", productArray[0].productNumber);
        for(int i=0; i<10; i++)
        {
            fprintf(f2, ",%s", productArray[0].dateArray[i]);
        }
        for(int i=1; i<num; i++)
        {
            INSTALLATION_SORT(i, productArray);

            fprintf(f2, "%s,", productArray[i].productNumber);
            for(int j=0; j<10; j++)
            {
                fprintf(f2, "%s,", productArray[i].dateArray[j]);
            }
            fprintf(f2, "%s", productArray[i].region);
        }
    }
    fclose(f2);
}
